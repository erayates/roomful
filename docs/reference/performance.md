...
- Set `CAHOOTS_REDIS_URL` on two relay instances that share the same Redis deployment.
- Run `pnpm --filter @cahoots/relay benchmark:redis` with that Redis URL in the environment.
...
- Run `pnpm --filter @cahoots/relay benchmark:load:steady` for the single-room 100-peer baseline.
- Run `pnpm --filter @cahoots/relay benchmark:load:scale -- --redis-url redis://127.0.0.1:6379/0` for the 500-peer, 50-room, 3-relay cluster scenario.
- Run `pnpm --filter @cahoots/relay benchmark:load:soak -- --redis-url redis://127.0.0.1:6379/0` for the 30-minute soak and inspect `benchmarks/results/<run-id>/report.md`.
