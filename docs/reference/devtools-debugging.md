...
`@roomful/devtools` now ships a browser DevTools extension bundle for Chrome and Firefox.
...
- `roomful-devtools-chrome.zip` store-ready Chrome archive
- `roomful-devtools-firefox.zip` store-ready Firefox archive
- `listings/` store listing copy and manual QA notes

The panel reads page diagnostics from the bridge injected by the core SDK at `window.__roomful_devtools__`.
...
Logs are console-backed and emitted as `console.info`, `console.warn`, or `console.error` with a `[Roomful]` prefix plus a structured payload. Every payload includes:
