...
Run the self-hostable `@cahoots/relay` signaling server locally for WebRTC validation:
...
- If WebRTC peers do not connect, verify `relayUrl` points to a reachable `@cahoots/relay` instance and check browser console ICE errors.
