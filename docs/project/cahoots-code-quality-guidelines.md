...
> **Context:** Cahoots is a framework-agnostic, zero-backend, open-source TypeScript library.  
...
Most TypeScript guidelines are written for applications. Cahoots is a library. The constraints are fundamentally different.

| Concern | Application (e.g. Zyora) | Library (Cahoots core) |
|---|---|---|
...
Safari 14 is in the support matrix. ES2019 is the highest common denominator that does not require polyfills for the features Cahoots uses. Do not raise this target without checking the browser support table first.

### Why `declaration: true`?

Cahoots ships types alongside the compiled output. Users get full autocomplete and type checking without any extra setup. This is non-negotiable for a TypeScript-first library.

---

## 3. Dependency Rules

### Core Package (`@cahoots/core`) — Zero Runtime Dependencies
...
**No Zod in core.** Validate with manual type guards (see Section 5). The relay server (`@cahoots/relay`) runs on Node.js where bundle size is irrelevant — Zod is fine there.

### Framework Adapters (`@cahoots/react`, `@cahoots/vue`, `@cahoots/svelte`)
...
// package.json — @cahoots/react
{
  ...
  "dependencies": {
    "@cahoots/core": "workspace:*"
  }
}
```

### Relay Server (`@cahoots/relay`) — Node.js Only
...
const peer = this.peers.get(peerId);
if (!peer) {
  this.emit('error', new CahootsError('PEER_NOT_FOUND', `No peer with id: ${peerId}`));
  return;
}
```
...
Cahoots has two real trust boundaries. Everything that crosses them is `unknown` and must be validated before use. Everything inside the library, typed by TypeScript, must never be defensively re-checked.
...
export function isCahootsMessage(data: unknown): data is CahootsMessage {
  if (!isObject(data)) return false;
  return (
...
channel.onmessage = (event: MessageEvent) => {
  ...
  if (!isCahootsMessage(raw)) {
    this.log('warn', 'Received malformed CahootsMessage, ignoring.', raw);
    return;
  }

  // raw is CahootsMessage from here on — no more checks needed downstream
...
```typescript
// @cahoots/relay — Node.js, Zod is fine
import { z } from 'zod';
...
Unlike Zyora (known Next.js + Node 20+ stack), Cahoots runs in genuinely unknown environments. Browser guards are sometimes required — but must be precise and documented.
...
### `crypto.randomUUID()` — Requires a Guard in Cahoots

Unlike in Zyora (Node 20+ guaranteed), Cahoots targets Node 18 and Safari 14 where `crypto.randomUUID()` may not exist.

```typescript
// ❌ WRONG for Cahoots — randomUUID is not available everywhere in the support matrix
const id = crypto.randomUUID();

// ✅ CORRECT for Cahoots — polyfill inline or detect
...
This is one of the few legitimate uses of an environment guard in Cahoots — and it is centralized in one place, not scattered across the codebase.
...
console.log('Cahoots loaded');             // side effect at module level
...
Every exported type, function signature, and class method is a contract with every Cahoots user. Changing a parameter type, removing an option, or renaming an event is a **breaking change**.
...
All errors thrown by Cahoots must be instances of `CahootsError` with a typed code. This allows users to handle specific error cases programmatically.
...
export class CahootsError extends Error {
  readonly code: CahootsErrorCode;

  constructor(code: CahootsErrorCode, message: string) {
    super(message);
...
room.on('error', (err) => {
  if (err instanceof CahootsError && err.code === 'ROOM_FULL') {
    showRoomFullDialog();
  }
});
```
...
```typescript
// ❌ UNHELPFUL
throw new CahootsError('INVALID_STATE', 'Invalid state');
...
> Paste this block into `.claude/CLAUDE.md` for Cahoots development.

```
CAHOOTS LIBRARY CODE RULES
============================

CONTEXT
────────
Cahoots is a zero-dependency, framework-agnostic TypeScript library.
...
───────────────────────────────
- @cahoots/core → zero runtime dependencies. Period.
- @cahoots/react/vue/svelte → framework as peerDependency only.
- @cahoots/relay → Node.js only, Zod and ws are fine here.
...
TRUST BOUNDARIES
─────────────────
There are exactly two trust boundaries in Cahoots core:
...
ENVIRONMENT GUARDS
──────────────────
Cahoots targets Safari 14 and Node 18 — environment guards ARE sometimes required.
Rules:
...
ERROR HANDLING
───────────────
- All thrown errors must be instances of CahootsError with a typed code
...
- [ ] Any new entry in `dependencies` in `@cahoots/core`? → Reject unless explicitly approved
...
- [ ] Error thrown with a raw `new Error()` instead of `CahootsError`? → Fix
...
| Error to throw | `new CahootsError(code, descriptive message)` |

---

*This document governs Cahoots library development. Application-level guidelines (Zyora) apply separately to apps that consume Cahoots — not to the library itself.*
