...
describe('relay cli', () => {
  ...
  it('returns an error result for invalid redis urls', () => {
    expect(
      resolveRelayCliOptions(
        {
          CAHOOTS_REDIS_URL: 'not-a-url',
        },
        [],
      ),
    ).toEqual({
      error: 'Invalid CAHOOTS_REDIS_URL value "not-a-url".',
    });
  });
  ...
  it('prints help output without starting the relay', async () => {
    ...
    const { processLike } = createFakeProcess({}, ['node', 'cahoots-relay', '--help']);
    ...
    expect(processLike.stdout.write).toHaveBeenCalledWith(
      expect.stringContaining('Usage: cahoots-relay [options]'),
    );
  });

  it('prints the package version without starting the relay', async () => {
    const createServer = vi.fn();
    const { processLike } = createFakeProcess({}, ['node', 'cahoots-relay', '--version']);
    ...
    expect(createServer).not.toHaveBeenCalled();
    expect(processLike.stdout.write).toHaveBeenCalledWith(
      `cahoots-relay ${readExpectedRelayPackageVersion()}\n`,
    );
  });
  ...
  it('resolves the documented CAHOOTS_* environment variables', () => {
    const resolved = resolveRelayCliOptions(
      {
        CAHOOTS_PORT: '9000',
        CAHOOTS_MAX_ROOM_SIZE: '200',
        CAHOOTS_CORS_ORIGIN: 'https://app.example.com',
        CAHOOTS_AUTH_SECRET: 'top-secret',
      },
      [],
    );
  ...
  it('prefers an explicit --port flag over CAHOOTS_PORT and PORT', () => {
    const resolved = resolveRelayCliOptions(
      {
        CAHOOTS_PORT: '9000',
        PORT: '8000',
