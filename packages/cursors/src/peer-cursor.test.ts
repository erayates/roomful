...
describe('PeerCursor', () => {
  it('renders as a decorative absolute-positioned cursor with smooth movement styles', () => {
    ...
    expect(root?.getAttribute('data-roomful-peer-cursor-style')).toBe('arrow');
    expect(root?.getAttribute('data-roomful-peer-cursor-color')).toBe('#123456');
  ...
  it.each([
    ...
    const marker = container.querySelector('[data-roomful-peer-cursor-marker="true"]');
    expect(marker).not.toBeNull();
    expect(marker?.tagName.toLowerCase()).toBe('svg');
    expect(marker?.getAttribute('data-roomful-peer-cursor-marker-style')).toBe(expectedStyle);
  });
  ...
  it('waits 3 seconds after becoming idle before fading the label', async () => {
    ...
    const label = container.querySelector('[data-roomful-peer-cursor-label="true"]');
  ...
  it('updates marker and label colors inline', () => {
    ...
    expect(root?.getAttribute('data-roomful-peer-cursor-color')).toBe('#123456');
    expect(label?.getAttribute('data-roomful-peer-cursor-label-color')).toBe('#123456');
  });
});
