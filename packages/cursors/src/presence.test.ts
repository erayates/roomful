...
describe('PresenceBar', () => {
  it('renders all peers from room presence, including the local user', () => {
    ...
    const root = container.querySelector('[data-cahoots-presence-bar="true"]');

    expect(root).not.toBeNull();
    expect(screen.queryByText('Ada Lovelace')).not.toBeNull();
    expect(screen.queryByText('Bob Stone')).not.toBeNull();
    expect(
      container.querySelector('[data-cahoots-presence-peer="self"]')?.getAttribute('title'),
    ).toBe('Ada Lovelace');
    expect(
      container.querySelector('[data-cahoots-presence-peer="peer-b"]')?.getAttribute('title'),
    ).toBe('Bob Stone');
  });
  ...
  it('renders avatar images and initials fallbacks inside name chips', () => {
    ...
    const { container } = renderPresenceBar();
    const image = container.querySelector('[data-cahoots-presence-avatar-image="true"]');
    const fallback = container.querySelector('[data-cahoots-presence-avatar-fallback="true"]');
  ...
  it('hides extra peers behind a +N overflow chip', () => {
    ...
    expect(container.querySelector('[data-cahoots-presence-overflow="true"]')?.textContent).toBe(
      '+2',
    );
    expect(
      container.querySelector('[data-cahoots-presence-overflow="true"]')?.getAttribute('title'),
  ...
  it('animates peers entering and leaving', async () => {
    ...
    expect(
      container
        .querySelector('[data-cahoots-presence-peer="peer-b"]')
        ?.getAttribute('data-cahoots-presence-phase'),
    ).toBe('entering');
    ...
    expect(container.querySelector('[data-cahoots-presence-peer="peer-b"]')).toBeNull();
  });
});
...
describe('PresenceAvatars', () => {
  it('renders visible avatar circles and a +N overflow badge', () => {
    ...
    const visiblePeers = Array.from(container.querySelectorAll('[data-cahoots-presence-peer]'));

    expect(container.querySelector('[data-cahoots-presence-avatars="true"]')).not.toBeNull();
