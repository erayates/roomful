...
describe('SelectionHighlight', () => {
  it('falls back to span injection and restores the original DOM on unmount', () => {
    ...
    const highlights = Array.from(
      document.querySelectorAll('[data-cahoots-selection-highlight="true"]'),
    );

    expect(highlights).toHaveLength(2);
    expect(highlights.map((highlight) => highlight.textContent)).toEqual(['a ', 'Lovelace']);
    expect(highlights[0]?.getAttribute('data-cahoots-selection-highlight-peer')).toBe('peer-a');
    expect(document.getElementById('editor')?.textContent).toBe('Ada Lovelace writes tests');

    unmount();

    expect(document.querySelector('[data-cahoots-selection-highlight="true"]')).toBeNull();
  ...
  it('uses the CSS custom highlight API when available and cleans up injected styles', () => {
    ...
    const styleElement = document.head.querySelector(
      `[data-cahoots-selection-highlight-style="${highlightName}"]`,
    );

    expect(String(highlightName)).toContain('cahoots-selection');
    ...
    expect(
      document.head.querySelector(`[data-cahoots-selection-highlight-style="${highlightName}"]`),
    ).toBeNull();
  });
});
