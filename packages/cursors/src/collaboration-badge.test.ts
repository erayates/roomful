...
describe('CollaborationBadge', () => {
  it('renders an absolute-positioned badge from peer and position props', () => {
    ...
    const badge = container.querySelector('[data-roomful-collaboration-badge="true"]');

    expect(badge).not.toBeNull();
    expect(badge?.getAttribute('aria-label')).toBe('Bob Stone is editing');
    expect(badge?.getAttribute('data-roomful-collaboration-badge-peer')).toBe('peer-b');
    expect(badge?.getAttribute('data-roomful-collaboration-badge-color')).toBe('#2244cc');
  ...
  it('removes the badge DOM node on unmount and falls back to top-right positioning', () => {
    ...
    expect(container.querySelector('[data-roomful-collaboration-badge="true"]')).toBeNull();
  });
});
