...
describe('FloatingReaction', () => {
  it('renders the emoji with animation styles at the correct normalized position', () => {
    ...
    const element = container.querySelector('[data-cahoots-floating-reaction="true"]');

    expect(element).not.toBeNull();
    expect((element as HTMLElement).style.left).toBe('25%');
    expect((element as HTMLElement).style.top).toBe('75%');
    expect((element as HTMLElement).style.animation).toContain('cahoots-floating-reaction-float');
    ...
    expect(
      container.querySelector('[data-cahoots-floating-reaction-styles="true"]'),
    ).not.toBeNull();
  });
  ...
  it('honors animation delay and waits before removing itself', async () => {
    ...
    expect(container.querySelector('[data-cahoots-floating-reaction="true"]')).not.toBeNull();
