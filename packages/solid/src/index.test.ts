...
describe('useHistory', () => {
  it('returns a timeline accessor and controls and reflects captures and remote entries', async () => {
    ...
    const { result } = renderHook(useHistory, {
      wrapper: createProviderWrapper('history-room'),
    });
    ...
    // A remote timeline change is reflected in the accessor.
    historyEngine.emit([
      createTimelineEntry('seed', { action: 'draw' }),
  ...
  it('skips deep-equal timeline snapshots', () => {
    ...
    const { result } = renderHook(useHistory, {
      wrapper: createProviderWrapper('history-reactivity'),
    });
