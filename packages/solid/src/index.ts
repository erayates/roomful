 ...
 * @returns The threads accessor plus add/reply/resolve/reopen and filter helpers.
 */
export function useComments<TPresence extends PresenceData = PresenceData>(
  ...
  const [threads, setThreads] = createSignal(readCommentsSnapshot(room, commentsEngine, cacheRef));

  onMount(() => {
    const sync = (): void => {
      const nextSnapshot = readCommentsSnapshot(room, commentsEngine, cacheRef);
      setThreads(() => nextSnapshot);
    };
 ...
 * @returns The proposals and pending accessors plus `approve`/`reject`/`propose`.
 */
export function useAgentApprovals<TPresence extends PresenceData = PresenceData>(
  ...
  const initialSnapshot = readApprovalsSnapshot(room, approvalsEngine, cacheRef);
  const [proposals, setProposals] = createSignal(initialSnapshot);
  const [pending, setPending] = createSignal(filterPendingProposals(initialSnapshot));

  onMount(() => {
    const sync = (): void => {
      const nextSnapshot = readApprovalsSnapshot(room, approvalsEngine, cacheRef);
      setProposals(() => nextSnapshot);
      setPending(() => filterPendingProposals(nextSnapshot));
    };
 ...
 *
 * @typeParam TPresence - The room presence shape.
 * @returns The active fields accessor plus `setActiveField` and `getFieldPeers`.
 */
...
>(): UseFieldPresenceResult {
  ...
  const [fields, setFields] = createSignal(
    readFieldPresenceSnapshot(room, fieldPresenceEngine, cacheRef),
  );

  onMount(() => {
    const sync = (): void => {
      const nextSnapshot = readFieldPresenceSnapshot(room, fieldPresenceEngine, cacheRef);
      setFields(() => nextSnapshot);
    };
