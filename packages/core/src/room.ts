...
const DIAGNOSTIC_PING_EVENT = '__cahoots:diag:ping__';
const DIAGNOSTIC_PONG_EVENT = '__cahoots:diag:pong__';
...
function isCahootsError(value: unknown): value is CahootsError {
  return value instanceof CahootsError;
}
...
function toTransportError(error: unknown): CahootsError {
  if (isCahootsError(error)) {
    return error;
  }
...
function normalizeTransportSerializationResult(
  ...
  if (!result.ok) {
    throw createCahootsError('ENCRYPTION_ERROR', result.error, false, {
...
export class RoomImpl<TPresence extends PresenceData = PresenceData> implements Room<TPresence> {
  ...
  public getYDoc(): CahootsYjsProvider['doc'] {
    return this.getOrCreateYjsController().doc;
  }

  public getYProvider(): CahootsYjsProvider {
  ...
  private emitPeerEncryptionModeError(peerId: string, reason: string, cause?: unknown): void {
    ...
    this.emitRoomError(
      createCahootsError('ENCRYPTION_ERROR', reason, true, {
        source: 'room-encryption',
  ...
  private emitPeerDecryptionError(peerId: string, error: CahootsError): void {
  ...
  private handleTransportErrorSignal(payload: { error: CahootsError }): void {
  ...
  private failInitialConnect(error: unknown): never {
    const cahootsError = toTransportError(error);
    this.connectStartedAt = null;
    this.unregisterUnloadHandlers();
    this.stopPresenceHeartbeat();
    this.clearRemoteState();
    ...
    if (cahootsError.code === 'ROOM_FULL') {
      this.roomEventEmitter.emit('room:full', undefined);
    }
    this.emitRoomError(cahootsError);
    throw cahootsError;
  }
  ...
  private emitRoomError(error: CahootsError): void {
