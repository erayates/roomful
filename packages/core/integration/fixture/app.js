...
function getRenderedCursorSnapshot() {
  ...
  return Array.from(board.querySelectorAll('[data-cahoots-peer-cursor]')).map((node) => {
    const marker = node.querySelector('[data-cahoots-cursor-marker]');
    const label = node.querySelector('[data-cahoots-cursor-label]');
    return {
      ...
      style: node.getAttribute('data-cahoots-cursor-style'),
      markerTag: marker?.tagName.toLowerCase() ?? null,
      markerStyle: marker?.getAttribute('data-cahoots-cursor-marker-style') ?? null,
      markerColor: marker?.getAttribute('data-cahoots-cursor-marker-color') ?? null,
...
function instrumentRtcChannel(channel) {
  if (channel.__cahootsInstrumented) {
    return;
  }

  channel.__cahootsInstrumented = true;
